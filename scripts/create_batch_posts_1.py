...
# Add current directory to path to import local scripts if needed,
# but we'll call the script via subprocess or just import functions if simpler.
# Actually, I'll just write the files directly.
...
def update_index(posts):
    ...
    if insert_idx == 0:
        insert_idx = len(lines)  # Fallback

    new_rows = []
    for post in posts:
        # | Post ID | Name | Link | Status | Type | Product | Depends On | Dependency Name | Expert |
        # Truncate title if too long for cleaner table? No, keep it.
        # Format link: [post](posts/YYYY/MM/YYYY-MM-DD_[postid]_slug/post-[slug]-[postid].md)
        today = datetime.date.today().strftime("%Y-%m-%d")
    ...
    # Update Next IDs
    # Next business post: 2026-B-016 -> 2026-B-020
    # Next tech post: 2026-T-016 -> 2026-T-016 (no change)
